...
// ✅ ADDED getDocs to enable data fetching
...
const mockConsultantData = [
    {
        "id": "mock-101", // Changed to string for consistency
        "name": "Dr. Elara Vance",
        "title": "Lead Smart Contract Auditor",
        "specialty": "Smart Contract Security", // Adjusted specialty for better filtering
        "rate": "1000 ALGO / hr",
        "rating": 4.9,
        "experience": 8,
    ...
    },
    {
        ...
        "specialty": "DeFi Strategy & Tokenomics", // Adjusted specialty for better filtering
...
// --- FILE UPLOAD COMPONENT (Fixed to use inputStyle class) ---
...
// --- CONSULTANT REGISTRATION FORM COMPONENT (FIXED) ---
const ConsultantRegistrationForm = ({ onClose, onNewConsultant }) => {
    const [formData, setFormData] = useState({
        ...
        achievements: '', 
        logoFile: null, 
        otherSpecialty: '', // State for custom specialty input
    });
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.name || !formData.title || !formData.email || !formData.rate || (formData.specialty === 'Other' && !formData.otherSpecialty)) {
        ...
        const newConsultantData = {
            ...
            // Generate a temporary mock ID for local display
            id: `temp-${Date.now()}`, 
            rating: 5.0, 
        ...
        try {
            // ✅ REAL BACKEND PROCESS: Add the new consultant to Firestore
            // NOTE: In a real app, you would upload the logoFile to Storage here before adding the document.
            // For now, we use the simulatedLogoUrl.
            const docRef = await addDoc(collection(db, "consultants"), newConsultantData);
            
            // Use the Firestore ID for the new consultant object
...
// --- POST CONSULTATION FORM COMPONENT (REVIEW) (Unchanged) ---
const PostConsultationForm = ({ consultant, userId, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Uses the globally defined appId
        const collectionPath = `artifacts/${appId}/users/${userId}/consultation_reviews`;
        ...
        try {
            // In a real app, you would add the document here:
...
// --- CONSULTANT PROFILE PAGE COMPONENT (FINAL FIXED UI) (Unchanged) ---
const ConsultantProfilePage = ({ consultant, onBack, userId }) => {
    ...
    const backgroundUrl = "https://placehold.co/1200x200/2C7C7C/FFFFFF?text=PROFILE+BANNER"; 
    ...
    return (
        <>
            <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden mb-10">
                ...
                <div className="relative">
                    ...
                    {/* Logo - Larger size, positioned clearly over the banner/content break */}
                ...
                <div className="p-8 pt-20 space-y-8 sm:pt-24"> 
...
// --- CONSULTANT CARD COMPONENT (LIST ITEM) (Unchanged) ---
    ...
    const filteredConsultants = useMemo(() => {
        return localConsultantData
            .filter(consultant => {
                ...
                // Fixed field matching to look for inclusion in the specialty string
                const matchesField = selectedField === 'All Specialties' || (consultant.specialty && consultant.specialty.toLowerCase().includes(selectedField.toLowerCase().replace(' ', '')) ); 
